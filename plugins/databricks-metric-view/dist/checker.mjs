...
var FEATURE_DOCS = "https://docs.databricks.com/gcp/en/uc-semantics/metric-views/feature-availability";
var METADATA_DOCS = "https://docs.databricks.com/aws/en/uc-semantics/agent-metadata";
var MATERIALIZATION_DOCS = "https://docs.databricks.com/aws/en/uc-semantics/metric-views/materialization";
...
var FIELD_KEYS = /* @__PURE__ */ new Set(["name", "expr", "comment", "display_name", "format", "synonyms"]);
var MEASURE_KEYS = /* @__PURE__ */ new Set([...FIELD_KEYS, "window"]);
var JOIN_KEYS = /* @__PURE__ */ new Set(["name", "source", "on", "using", "cardinality", "joins", "rely"]);
var PARAMETER_KEYS = /* @__PURE__ */ new Set(["name", "data_type", "default"]);
var WINDOW_KEYS = /* @__PURE__ */ new Set(["order", "range", "semiadditive", "offset"]);
var WILDCARD_EXPRESSION = /^(?:`[^`]+`|[A-Za-z_][\w$]*)(?:\.(?:`[^`]+`|[A-Za-z_][\w$]*))*\.\*(?:\s+EXCEPT\s*\(\s*(?:`[^`]+`|[A-Za-z_][\w$]*)(?:\s*,\s*(?:`[^`]+`|[A-Za-z_][\w$]*))*\s*\))?$/i;
var WILDCARD_PREFIX = /^(?:`[^`]+`|[A-Za-z_][\w$]*)(?:\.(?:`[^`]+`|[A-Za-z_][\w$]*))*\.\*/i;
...
function parseRuntime(value) {
  ...
  const minor = match[2] === void 0 ? major >= 18 ? Number.POSITIVE_INFINITY : 0 : Number(match[2]);
  return [major, minor];
}
function runtimeLessThan(actual, minimum) {
...
var ValidationContext = class {
  constructor(document, lineCounter, options) {
  ...
  runtime;
  version;
  explicitOutputNames = /* @__PURE__ */ new Map();
  fieldExpressions = /* @__PURE__ */ new Map();
  parameterTypes = /* @__PURE__ */ new Map();
  hasWildcardFields = false;
  hasWildcardMeasures = false;
  add(code, severity, path, message, extras = {}) {
    let nodePath = path;
    let node = this.document.getIn(nodePath, true);
    while (!node?.range && nodePath.length > 0) {
      nodePath = nodePath.slice(0, -1);
      node = this.document.getIn(nodePath, true);
    }
    ...
    for (const key of Object.keys(value)) {
      if (!allowed.has(key)) {
        this.add(
          ...
          {
            suggestion: this.options.allowUnknownFields ? "Verify the field against current Databricks documentation." : "Correct a possible typo, update the checker, or rerun with --allow-unknown after reviewing current Databricks documentation.",
            docsUrl: YAML_DOCS
          }
        );
      }
  ...
  feature(path, featureName, minimumRuntime, options = {}) {
...
function requireObject(ctx, value, path, label) {
  if (!isObject(value)) {
...
function requireArray(ctx, value, path, label, nonEmpty = true) {
  if (!Array.isArray(value)) {
...
function requireString(ctx, value, path, label) {
  if (!nonBlank(value)) {
...
function stringList(ctx, value, path, label, nonEmpty = true) {
...
function validateDecimalPlaces(ctx, value, path) {
  const decimal = requireObject(ctx, value, path, "decimal_places");
  if (!decimal) return;
  ctx.unknownFields(decimal, /* @__PURE__ */ new Set(["type", "places"]), path);
  ...
  if (type === "max" || type === "exact") {
    if (!Number.isInteger(decimal.places) || decimal.places < 0 || decimal.places > 10) {
      ctx.add(
...
function validateFormat(ctx, value, path) {
  const format = requireObject(ctx, value, path, "format");
  if (!format) return;
  const type = requireString(ctx, format.type, [...path, "type"], "format.type");
  const commonNumeric = /* @__PURE__ */ new Set(["type", "decimal_places", "hide_group_separator", "abbreviation"]);
  const allowedByType = {
    number: commonNumeric,
    currency: /* @__PURE__ */ new Set([...commonNumeric, "currency_code"]),
    percentage: /* @__PURE__ */ new Set(["type", "decimal_places", "hide_group_separator"]),
    byte: /* @__PURE__ */ new Set(["type", "decimal_places", "hide_group_separator"]),
    date: /* @__PURE__ */ new Set(["type", "date_format", "leading_zeros"]),
    date_time: /* @__PURE__ */ new Set(["type", "date_format", "time_format", "leading_zeros"])
  };
  ...
  if (type === "date_time" && (!Object.hasOwn(format, "date_format") || format.date_format === "no_date") && (!Object.hasOwn(format, "time_format") || format.time_format === "no_time")) {
    ctx.add(
...
function registerOutputName(ctx, name, path, kind) {
...
function isWildcardExpression(expression) {
  return WILDCARD_EXPRESSION.test(expression.trim());
}
function validateWindow(ctx, value, path) {
...
function validateWindowParameter(ctx, name, path) {
...
function validateColumns(ctx, value, path, kind) {
...
function validateParameters(ctx, value, path) {
...
function validateJoins(ctx, value, path, parentCardinality) {
  const joins = requireArray(ctx, value, path, "joins");
  if (!joins) return;
  const siblingNames = /* @__PURE__ */ new Map();
  joins.forEach((entry, index) => {
    ...
    if (Object.hasOwn(join, "cardinality")) {
      ...
      if (suppliedCardinality && !["many_to_one", "one_to_many"].includes(suppliedCardinality)) {
        ctx.add(
        ...
        cardinality = suppliedCardinality;
      }
    }
    if (parentCardinality && cardinality && parentCardinality !== cardinality) {
      const parentIsOneToMany = parentCardinality === "one_to_many";
      ctx.add(
        parentIsOneToMany ? "ONE_TO_MANY_DESCENDANT_CARDINALITY" : "MANY_TO_ONE_DESCENDANT_CARDINALITY",
        "error",
        [...itemPath, "cardinality"],
    ...
    if (Object.hasOwn(join, "rely")) {
      ...
      if (rely) {
        ctx.unknownFields(rely, /* @__PURE__ */ new Set(["at_most_one_match"]), [...itemPath, "rely"]);
        if (Object.hasOwn(rely, "at_most_one_match") && typeof rely.at_most_one_match !== "boolean") {
          ctx.add(
            "BOOLEAN_REQUIRED",
            "error",
...
function validateReferenceList(ctx, value, path, label, known, hasWildcard) {
  ...
  references?.forEach((reference, index) => {
    ...
    if (!known.has(key)) {
      ctx.add(
        ...
        [...path, index],
        hasWildcard ? `${JSON.stringify(reference)} is not an explicit output but might be supplied by a wildcard; source columns were not inspected.` : `${JSON.stringify(reference)} does not match an explicit metric-view output.`,
        { docsUrl: MATERIALIZATION_DOCS }
      );
    }
  });
...
function validateClusterBy(ctx, value, path) {
  const cluster = requireObject(ctx, value, path, "cluster_by");
  if (!cluster) return;
  ctx.unknownFields(cluster, /* @__PURE__ */ new Set(["cols", "auto"]), path);
...
function validateMaterialization(ctx, value, path) {
  ...
  ctx.unknownFields(materialization, /* @__PURE__ */ new Set(["schedule", "mode", "materialized_views"]), path);
  ...
  const viewNames = /* @__PURE__ */ new Map();
  ...
  views.forEach((entry, index) => {
    ...
    ctx.unknownFields(
      view,
      /* @__PURE__ */ new Set(["name", "type", "dimensions", "measures", "cluster_by", "partition_by"]),
      itemPath
    );
...
function validateVersion(ctx, root) {
  ...
  const node = ctx.document.get("version", true);
  const raw = node?.source === void 0 ? String(root.version) : String(node.source);
  if (raw !== "0.1" && raw !== "1.1") {
    ctx.add(
...
function syntaxResult(diagnostics, options) {
  ...
  return {
    ...
    infoCount: infos,
    diagnostics,
    disclaimer: errors === 0 ? "Local checks passed; Databricks SQL expressions, catalog objects, permissions, source data, join cardinality, and analyzer behavior were not validated." : "Local checks failed; the definition was not submitted to Databricks. Databricks SQL expressions and catalog semantics were not validated."
  };
}
function validateMetricViewYaml(source, options = {}) {
  ...
  for (const document2 of documents) {
    for (const error2 of document2.errors) {
      const position = lineCounter.linePos(error2.pos[0]);
      syntaxDiagnostics.push({
        code: error2.code || "YAML_PARSE_ERROR",
        severity: "error",
        path: "$",
        line: position.line,
        column: position.col,
...
function parseArguments(args) {
  ...
  compute ??= runtimeVersion ? "dbr" : void 0;
  const options = {
    ...compute ? { compute } : {},
...
var TOOL_NAME = "check_databricks_metric_view_yaml";
var toolDefinition = {
  name: TOOL_NAME,
...
function validateToolArguments(value) {
  ...
  if (hasFile && !isAbsolute(value.file)) {
    throw new Error("file must be an absolute path because the plugin runs from its installation directory");
  }
  if (value.compute !== void 0 && value.compute !== "sql-warehouse" && value.compute !== "dbr") {
    throw new Error("compute must be sql-warehouse or dbr");
  }
  if (value.runtime_version !== void 0 && typeof value.runtime_version !== "string") {
...
async function callTool(argumentsValue) {
  ...
  const yaml = typeof args.yaml === "string" ? args.yaml : await readYamlFile(args.file);
  const runtimeVersion = typeof args.runtime_version === "string" ? args.runtime_version : void 0;
  const compute = args.compute ?? (runtimeVersion ? "dbr" : void 0);
  const options = {
    ...compute ? { compute } : {},
    ...runtimeVersion ? { runtimeVersion } : {},
...
async function runMcpServer() {
  ...
  for await (const line of lines) {
    ...
    try {
      ...
      if (!isObject2(incoming)) throw new Error("Request must be a JSON object");
...
main().catch((error2) => {
  process.stderr.write(`metric-view-checker: ${formatCliError(error2)}
`);
  process.exitCode = 2;
});
